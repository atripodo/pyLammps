...
def first_and_second_shell_cordination_num(wr,shifted_box,conf,dist1=1.44,dist2=2.3):
    nc=wr.shape[0]
    npa=wr.shape[1]
...
def radial_distrib_func(wr,shifted_box,sample_size,dr=0.1):
    ...
    gr_tot=[]
    for conf in sample:
        rho=npa/(shifted_box[conf,0,1]*shifted_box[conf,1,1]*shifted_box[conf,2,1])
...
def displacement_analysis(r):
    ...
    for conf in range(1,nc):
        ...
        displ.append([np.mean(temp),np.mean(temp**2),0.6*np.mean(temp**4)/np.mean(temp**2)**2-1])
